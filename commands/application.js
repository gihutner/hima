...
module.exports = {
    ...
    execute(message, args) {
        ...
        if((commands === "application") && (args.length > 0) && (message.guild.members.cache.get(message.author.id).hasPermission("ADMINISTRATOR"))) {
...
// add a new suggestion into the db
            ...
            .setAuthor(`${the_users_name}#${the_users_discrim}`, `${the_users_avatar}`)
            ...
            client.channels.cache.get('742603951995879533').send(application_emb);


        } else if (args.length === 0) {
            const wrong_application = new Discord.MessageEmbed()
            .setDescription(`That was the incorrect usage. Try \`h.application [application info]\`.`)
